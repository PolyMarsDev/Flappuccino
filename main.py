...
def start():
    global height, beanMultiplier, beans, buttons, last_time, clicked, jump, dt, mouseX, mouseY
    last_time = time.time()
    clicked = jump = False
    height = dt = 0
    beanMultiplier = 5
    ...
    buttons[0].set_indicator('data/gfx/flap_indicator.png')
    buttons[0].set_price(5)   
    buttons[1].set_indicator('data/gfx/speed_indicator.png')
    buttons[1].set_price(5)   
    buttons[2].set_indicator('data/gfx/beanup_indicator.png')
...
def main():
    global player, clicked, jump, height, mouseY, dt, mouseX, keys, beanMultiplier, beans, buttons
    # variables
    rotOffset = -5
    ...
    while True:
        ...
        for button in buttons:
            ...
            if clicked and not player.dead and checkCollisions(mouseX, mouseY, 3, 3, buttonX, buttonY, button.sprite.get_width(), button.sprite.get_height()):
